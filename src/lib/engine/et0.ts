// Hargreaves ET0 (FAO-56 simplified). Adequate for central Chile.
// ET0 = 0.0023 * (Tmean + 17.8) * sqrt(Tmax - Tmin) * Ra
// Ra = extraterrestrial radiation in mm/day (eq 21 FAO-56).
...
export function calcularRadiacionExtraterrestre(
  ...
  const delta = 0.409 * Math.sin((2 * Math.PI * diaDelAno) / 365 - 1.39);
...
export function calcularEt0Hargreaves(
  ...
  if (tMax < tMin) throw new Error('tMax debe ser >= tMin');
