// Estadísticas NDVI/NDMI de una zona vía Process API (PNG 32×32 codificado).
// Extraído de app/api/zone-stats/route.ts para reutilizarlo desde el backfill
// (fallback por fecha cuando el Statistical API no está disponible).
...
export function bboxFromPolygon(polygon: GeoJSONPolygon): [number, number, number, number] {
...
export function decodePngMean(buf: Buffer): { r: number; g: number } | null {
...
export function mockNdviForPolygon(polygon: GeoJSONPolygon): number {
  const coords = polygon.coordinates[0];
...
export function mockNdmiForPolygon(polygon: GeoJSONPolygon): number {
  const coords = polygon.coordinates[0];
...
// Mean NDVI/NDMI for a zone polygon on a given date (one Process API call).
export async function getZoneStats(polygon: GeoJSONPolygon, date: string): Promise<ZoneStats> {
  if (isMockMode()) {
    return {
  ...
  return {
    ndvi: parseFloat(((avg.r / 255) * 2 - 1).toFixed(3)),
    ndmi: parseFloat(((avg.g / 255) * 2 - 1).toFixed(3)),
  };
}
